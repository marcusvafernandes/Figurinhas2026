...
export default function CompareTool({ myStickers }: CompareToolProps) {
  ...
  return (
    <div id="compare_tool_container" className="bg-white border border-slate-200/80 rounded-2xl p-5 md:p-6 shadow-sm flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-baseline">
          ...
          <span className="text-[10px] font-mono text-slate-400">Suporta códigos livres (ex: BRA-1, ARG12, FWC 2)</span>
        </div>
        <div className="relative">
          <textarea
            ...
            className="w-full px-4 py-3 border border-slate-200 rounded-2xl focus:border-emerald-500 focus:outline-none text-xs text-slate-700 placeholder-slate-400 leading-relaxed font-semibold shadow-xs"
          />
          {inputText && (
            <button
              type="button"
              onClick={() => setInputText('')}
              className="absolute bottom-3 right-3 p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-lg transition"
              title="Limpar"
            >
      ...
      <div className="flex flex-col gap-4">
        ...
        {inputText && (
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs">
            <div className="flex items-center gap-2">
              <span className="flex h-2 w-2 rounded-full bg-emerald-500" />
              <span className="text-slate-600 font-medium">
                Encontramos <strong className="text-slate-800 font-extrabold font-mono">{parsedAll.length}</strong> referências de figurinhas no seu texto.
              </span>
            </div>
            {parsedAll.length > 0 && (
              <span className="text-[10px] bg-slate-200 text-slate-700 px-2 py-0.5 rounded-md font-mono font-bold uppercase">
                Análise Concluída
              </span>
            )}
          </div>
        ...
        {matchedStickers.length > 0 && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-emerald-50 to-teal-50/30 border border-emerald-100 p-4 rounded-2xl">
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-emerald-700/85 font-semibold">
                Encontrado <strong className="font-extrabold text-emerald-800">{matchedStickers.length} match(es)</strong> perfeito(s). Compartilhe o resultado!
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleCopy}
                className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3 py-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-xl text-[11px] font-bold transition cursor-pointer shadow-xs active:scale-98"
              >
        ...
        {inputText ? (
          matchedStickers.length > 0 ? (
                ...
                return (
                  <div
                    ...
                    className={`relative border rounded-xl p-3 flex flex-col items-center justify-between text-center transition shadow-2xs ${
                      isSpecial 
                        ? 'bg-amber-50/60 border-amber-250 hover:border-amber-400' 
                        : 'bg-slate-50/40 border-slate-200/80 hover:border-slate-350'
                    }`}
                  >
                    {/* Tiny shiny icon */}
                    {isSpecial && (
                    ...
                    <div className="flex items-center gap-1 mb-1.5">
                      ...
                      <span className="text-[10px] font-black text-slate-500 font-mono tracking-wider">{sticker.teamCode}</span>
                    </div>

                    {/* Sticker code display */}
                    <div className="mb-1">
                      <span className={`text-sm font-black font-mono px-2 py-0.5 rounded-lg ${
                        isSpecial ? 'bg-amber-200/60 text-amber-950' : 'bg-slate-200/60 text-slate-800'
                      }`}>
                        {sticker.id}
                      </span>
                    </div>
                    ...
                    <div className="mt-1.5">
                      {mode === 'direct' ? (
                        <span className="text-[9px] font-black text-rose-650 bg-rose-50 px-1.5 py-0.5 rounded-md border border-rose-100/60 font-mono uppercase tracking-wide">
                          Faltando
                        </span>
                      ) : (
                        <span className="text-[9px] font-black text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-md border border-emerald-100/60 font-mono uppercase tracking-wide">
                          Tenho ({repeatedQty}x)
                        </span>
                      )}
                    </div>
            ...
            <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200 flex flex-col items-center justify-center gap-2">
              <span className="text-2xl">🔍</span>
              <p className="text-xs font-bold text-slate-600">
                Nenhum match encontrado para o texto fornecido.
              </p>
              <p className="text-[10px] text-slate-450 px-6 max-w-sm font-semibold leading-relaxed">
                {mode === 'direct' 
          ...
          <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-xs font-bold text-slate-700">Aguardando a colagem do texto para análise...</p>
            <p className="text-[10px] text-slate-400 px-6 max-w-sm leading-relaxed font-semibold">
      ...
      {notification && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-xl flex items-center gap-2.5 max-w-xs sm:max-w-md border border-slate-850 animate-bounce">
